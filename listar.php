<?php
// Incluir a conexão xom o banco de dados
include_once "conexao.php";

// Receber a página atual via método GET 
$pagina = filter_input(INPUT_GET, "pagina", FILTER_SANITIZE_NUMBER_INT);

// Verificar se a página da visualização
if (!empty($pagina)) {
    ...
    $query_result_pg = "SELECT usr.id, usr.nome, usr.email, 
    ...
    if (($result_usuarios) and ($result_usuarios->rowCount() != 0)) {
        ...
        while ($row_usuario = $result_usuarios->fetch(PDO:: FETCH_ASSOC)) {
            ...
            $dados .= "<td>$numero</td>";
            $dados .= "<td><a hferf='#' class='btn btn-outline-primary btn-sm' onclik='visUsuario($id)'>Visualizar</a>
             <a hferf='#' class='btn btn-outline-warning btn-sm' onclick='editUsuarioDados ($id)'>Editar</a> 
            <a hferf='#' class='btn btn-outline-danger btn-sm' onclick='apagarUsuarioDados ($id)'>Apagar</a></td>";
            $dados .= "</tr>";
        }
        $dados .= "</tbody>";
        $dados .= "</table>";
        ...
        // Loop para link das páginas anteriores
        ...
        $dados .="<li class='page-item'><a class='page-link' href='#' onclick='listaUsuarios ($quantidade_pg)'>Última</a></li>";

        $dados .= "</ul>";
        $dados .= "</nav>";

        $retorna = ['status' => true, 'dados' => $dados, 'quantidade-pg' => $quantidade_pg];
    } else {
        $retorna = ['status' => false, 'msg' => "<p style='colo: #f00;'>Erro: Nenhum usuário encontrado!</p>"];
    }
} else {
...
// Um arquivo JSON (JavaScript Object Notation) é um formato leve de troca de dados que utiliza uma sintaxe fácil de ler e escever. Ele é frequentemente usado para representar estruturas de dados simples, como pares chave-valor, arrays e objetos, e é comumente usado em comunicações entre servidores web e clientes, sendo fácil de entender tanto por humanos quanto por máquinas. 